import sys
import os
from models_eb import Decoder, Encoder, Nu_xz, Nu_z
...
# global parameters
parser.add_argument('--train_file', default=f'data/{run}/{ds}-train.hdf5', )
parser.add_argument('--val_file', default=f'data/{run}/{ds}-val.hdf5')
parser.add_argument('--test_file', default=f'data/{run}/{ds}-test.hdf5')
parser.add_argument('--results_folder_prefix', default=f'iVAE/output/{ds}_results_')
...
def sample_sentences(decoder, vocab, num_sentences, reconstruction=False, data=test_data):
    ...
    if reconstruction:
        ...
        z_x = z_x.ex[expand_int, :]
        sents = sents_batch.ex[expand_int, :].tolist()

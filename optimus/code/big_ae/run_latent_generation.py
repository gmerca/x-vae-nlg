...
from pytorch_transformers import GPT2Config, BertConfig
...
ALL_MODELS = sum((tuple(conf.pretrained_config_archive_map.keys()) for conf in (GPT2Config, BertConfig)), ())
...
def interpolate(model_vae, tokenizer_encoder, tokenizer_decoder, args, s1, s2):
    global n_found
    global n_repetitions

    latent_z1, coded_length1 = latent_code_from_text(s1, tokenizer_encoder, model_vae, args)
    latent_z2, coded_length2 = latent_code_from_text(s2, tokenizer_encoder, model_vae, args)
    ...
    for sent in result.values():
        sent = re.sub(r'\s([?,.!"](?:\s|$))', r'\1', sent.lower())
        no_stop_sent = ' '.join([el for el in sent.split() if el not in stopwords])
        ...
        if len(copy_df) != 0:
            n_found += 1
        elif no_stop_sent in generated_non_stop:
            n_repetitions += 1
...
def main():
    ...
    ex['topic'] = ex['topic'].apply(lambda topics: [topic.strip() for topic in topics])
    ...
    for i in range(len(ex)):
        ...
        topics = [utils.recognize_entities(f)  for f in facts] # extract topic from cleaned fact
        topics = [item for sublist in topics for item in sublist]
        for el in q_topics:
            topic_dict[el].update(topics)

    ex = ex.loc[(ex['explanation'].map(len) > 1)] # explanations with at least 2 facts
    e = 'explanation'; main_types = ['CENTRAL', 'LEXGLUE']

    global n_found; global n_repetitions; global n_eval
    refs = []
    ...
    for i in tqdm(range(len(ex))):
        ...
        df_list = []
        for t in f_types:
            df_list.append(same_types[same_types['table_name'] == t])
        ...
        for j, df in enumerate(df_list):
            ...
            for pair_of_sents in zip(*(iter(f_facts),) * 2):
                ...
                n_eval += 1
                refs += [r_facts] * 12
                interpolate(model_vae, tokenizer_encoder, tokenizer_decoder, args, s1, s2)

    _, _, bert_sc = score(generated_s, refs, rescale_with_baseline=True)
